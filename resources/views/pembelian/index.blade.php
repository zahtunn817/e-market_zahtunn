...
@section('content')
    ...
    <a href="{{ url('transaksiPembelian') }}" class="btn btn-primary mb-3">
        Tambah {{ $page }}
    </a>
...
@push('script')
    ...
    <script>
        ...
        $('.delete-data').on('click', function(e) {
            e.preventDefault()
            let nama_pembelian = $(this).closest('tr').find('td:eq(2)').text()
            Swal.fire({
                title: `Apakah data <span style="color:red"><b>${nama_pembelian}</b></span> akan dihapus?`,
        ...
        $('#form{{ $page }}Modal').on('show.bs.modal', function(e) {
            ...
            const kode_pembelian = btn.data('kode_pembelian')
            ...
            if (mode === 'edit') {
                ...
                modal.find('#kode_pembelian').val(kode_pembelian)
